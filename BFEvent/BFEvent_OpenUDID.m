...
@interface BFEvent_OpenUDID (Private)
...
@implementation BFEvent_OpenUDID
...
//
+ (NSString *)value {
    return [BFEvent_OpenUDID valueWithError:nil];
}
...
+ (NSString *)valueWithError:(NSError **)error {
    ...
    for (int n = 0; n < kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB == nil) {
            ...
            NSDictionary *dict = [BFEvent_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID == nil) {
        if (mostReliableOpenUDID == nil) {
            ...
            //
            openUDID = [BFEvent_OpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid != nil && (myRedundancySlotPBid == nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [BFEvent_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
